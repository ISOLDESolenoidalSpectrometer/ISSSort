...
TimeSorter::TimeSorter( Calibration *mycal ){
	
	cal = mycal;
	
	LoadParametersSort(); //maybe also file to read param from file
	
}

void TimeSorter::Initialise(){
...
void TimeSorter::SortTree(){
	...
	/// A function to time sort the full TTree and apply an index
	output_file->cd();
	nb_idx = output_tree->BuildIndex( "time" );
...
bool TimeSorter::SetEntry( long long ts, long long ts_ext ) {
...
void TimeSorter::LoadParametersSort() { // maybe also file to read param from file
...
void TimeSorter::SortFile( std::string input_file_name,
	...
	std::cout << " Sorting: number of entries in calibrated tree = " << output_tree->GetEntries() << std::endl;
	log_file << " Sorting: number of entries in calibrated tree = " << output_tree->GetEntries() << std::endl;
	SortTree();
	...
	std::cout << "End TimeSort: time elapsed = " << time(NULL)-t_start << " sec." << std::endl;
	log_file << "End TimeSort: time elapsed = " << time(NULL)-t_start << " sec." << std::endl;
