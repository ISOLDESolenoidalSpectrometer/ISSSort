...
void Calibrator( std::string input_file_name, std::string output_file_name,
				std::string log_file_name, std::string cal_file_name ){
	...
	LoadParametersCali( common::n_module, cal_file_name );
	...
	output_tree->Branch( "entry_calib", &s_entry.energy,
						"energy/D:time_stamp/D:detector/I:side/I:strip/I:sync_flag/O");
	
	
	
	// Open Root input file.
	std::cout << "Calibrating file: " << input_file_name << std::endl;
	TFile* input_file = new TFile( input_file_name.data(), "read" );
	...
	// Find the tree from the input file
	TTree *input_tree= (TTree*) input_file->Get("iss");
	...
	std::cout << "\n Calibrator ...done with " << input_file_name << std::endl;
