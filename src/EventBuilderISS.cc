...
void DoEventBuild(std::string input_list_name, std::string output_file_name, std::string log_file_name, std::string param_file_name){
	...
	if (input_list.is_open()){
		...
		std::cout << " trying to use this TBranch name: "<<contenedor << std::cout;
		...
		for(int iter_f=0;iter_f<n_input_files;iter_f++){
			...
			if (input_file != 0){
				...
				TTree *input_tree= (TTree*) input_file->Get("ISS_calib");
				...
				for(long long i=0; i<n_entries; i++){
					...
					my_BPulseNb=s_entry_calib.BPulseNb;
					...
					// Let's look at data recorded within same beam pulse:4
					if(Trig==s_entry_calib.BPulseNb}
					   if(my_detector<4)  // STUB 0,1,2,3
					   {
						hitSTUB=true;
					}
					   
					   if(my_detector==4)  // Recoil
					   {
						hit_Recoil=true;
					}
					   if(my_detector==5)  // S1
